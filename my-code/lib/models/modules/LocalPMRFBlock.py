...
class LocalPolarizedMultiScaleReceptiveFieldSelfAttentionBlock_ExtendChannel(nn.Module):
    ...
    """
    def __init__(self, channel=512, kernels=[1, 3, 5, 7], group=1, dilation=2):
        """
...
class LocalPolarizedMultiScaleReceptiveFieldSelfAttentionBlock_SelectiveKernel_Parallel(nn.Module):
    ...
    """
    def __init__(self, channel, kernels=[1, 3, 5, 7], r=16, group=1, L=32, dilation=2):
        """
...
class LocalPolarizedMultiScaleReceptiveFieldSelfAttentionBlock_ExtendInnerProductVector(nn.Module):
    ...
    """
    def __init__(self, in_channel, channel, kernels=[1, 3, 5, 7], group=1, dilation=2):
        """
        ...
        self.ch_Wq_convs = nn.ModuleList([
            nn.Sequential(OrderedDict([
                ("conv", nn.Conv3d(channel, channel//(self.k * 2), kernel_size=k, padding=((k - 1) * dilation) // 2, dilation=dilation, groups=group)),
                ("bn", nn.BatchNorm3d(channel//(self.k * 2))),
                ("relu", nn.ReLU(inplace=True))
            ]))
            for k in kernels
        ])
        ...
        self.ch_Wk_convs = nn.ModuleList([
            nn.Sequential(OrderedDict([
                ("conv", nn.Conv3d(channel, 1, kernel_size=k, padding=((k - 1) * dilation) // 2, dilation=dilation, groups=group)),
                ("bn", nn.BatchNorm3d(1)),
                ("relu", nn.ReLU(inplace=True))
            ]))
            for k in kernels
        ...
        self.ch_softmax = nn.Softmax(1)
        # 定义通道注意力对分数矩阵的卷积Wz
        self.ch_Wz = nn.Conv3d(channel//(self.k * 2), channel, kernel_size=1)
        # 定义通道注意力的层归一化
    ...
    def forward(self, x):
        ...
        # 堆叠通道Wq
        ch_Wq = torch.stack(ch_Wq_outs, dim=-1)  # bs, c//(2*k), d, h, w, k
        # 计算通道Wk各分支
        ch_Wk_outs = [
            conv(x)
        ...
        # 转换维度
        ch_Wq = ch_Wq.reshape(bs, c//(self.k * 2), -1)  # bs, c//(2*k), d*h*w*k
        ch_Wk = ch_Wk.reshape(bs, -1, 1)  # bs, d*h*w*k, 1
        # 进行Softmax处理
        ch_Wk = self.ch_softmax(ch_Wk)  # bs, d*h*w*k, 1
        # 矩阵相乘
        ch_Wz = torch.matmul(ch_Wq, ch_Wk).unsqueeze(-1).unsqueeze(-1)  # bs, c//(2*k), 1, 1, 1
        # 计算通道注意力分数矩阵
        ...
        # 堆叠空间Wq
        sp_Wq = torch.stack(sp_Wq_outs, dim=1)  # bs, k, c//(2*k), d, h, w
        # 计算空间Wk各分支
        sp_Wk_outs = [
            conv(ch_out)
        ...
        # 堆叠空间Wk
        sp_Wk = torch.stack(sp_Wk_outs, dim=1)  # bs, k, c//(2*k), d, h, w
        # 转换维度
        sp_Wq = sp_Wq.reshape(bs, -1, d * h * w)  # bs, k*c//(2*k), d*h*w
        sp_Wk = sp_Wk.mean(-1).mean(-1).mean(-1).reshape(bs, 1, -1)  # bs, 1, k*c//(2*k)
        # 进行Softmax处理
        sp_Wk = self.sp_softmax(sp_Wk)  # bs, 1, k*c//(2*K)
        # 矩阵相乘
...
class LocalPolarizedMultiScaleReceptiveFieldSelfAttentionBlock_ExtendAttentionPoints(nn.Module):
    ...
    def __init__(self, in_channel, channel, kernels=[1, 3, 5, 7], group=1, dilation=2):
        ...
        self.ch_Wq_convs = nn.ModuleList([
            nn.Sequential(OrderedDict([
                ("conv", nn.Conv3d(channel, channel//self.k, kernel_size=k, padding=((k - 1) * dilation) // 2, dilation=dilation, groups=group)),
                ("bn", nn.BatchNorm3d(channel//self.k)),
                ("relu", nn.ReLU(inplace=True))
            ]))
            for k in kernels
        ])
        ...
        self.ch_Wv_convs = nn.ModuleList([
            nn.Sequential(OrderedDict([
                ("conv", nn.Conv3d(channel, channel, kernel_size=k, padding=((k - 1) * dilation) // 2, dilation=dilation,
                                   groups=group)),
                ("bn", nn.BatchNorm3d(channel)),
                ("relu", nn.ReLU(inplace=True))
            ]))
        ...
        # 将Wk的各分支加权为通道数为1
        self.ch_Wk_Conv = nn.Conv3d(self.k, 1, kernel_size=1)
        # 定义通道注意力的Softmax
        self.ch_softmax = nn.Softmax(1)
        # 定义通道注意力对分数矩阵的卷积Wz
        self.ch_Wz = nn.Conv3d(channel, self.k * channel, kernel_size=1)
        # 定义通道注意力的层归一化
        self.layer_norm = nn.LayerNorm((self.k * channel, 1, 1, 1))
        # 定义对注意力分数矩阵的Sigmoid激活函数
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        # 堆叠通道Wq
        ch_Wq = torch.stack(ch_Wq_outs, dim=1)  # bs, k, c//2, d, h, w
        # 计算通道Wk各分支
        ch_Wk_outs = [
            conv(x)
            for conv in self.ch_Wk_convs
        ...
        # 堆叠通道Wk
        ch_Wk = torch.cat(ch_Wk_outs, dim=1)  # bs, k, d, h, w
        ch_Wk = self.ch_Wk_Conv(ch_Wk)
        # 计算通道Wv各分支
        ch_Wv_outs = [
            conv(x)
        ...
        # 堆叠通道Wq
        ch_Wv = torch.stack(ch_Wv_outs, dim=1)  # bs, k, c, d, h, w
        # 转换维度
        ch_Wq = ch_Wq.reshape(bs, -1, d * h * w)  # bs, k * c//2, d*h*w
        ch_Wk = ch_Wk.reshape(bs, -1, 1)  # bs, d*h*w, 1
        # 进行Softmax处理
        ch_Wk = self.ch_softmax(ch_Wk)  # bs, d*h*w*k, 1
        # 矩阵相乘
        ch_Wz = torch.matmul(ch_Wq, ch_Wk).unsqueeze(-1).unsqueeze(-1)  # bs, k * c//2, 1, 1, 1
        # 计算通道注意力分数矩阵
        ch_score = self.sigmoid(self.layer_norm(self.ch_Wz(ch_Wz))).reshape(bs, -1, c, 1, 1, 1)  # bs, k, c, 1, 1, 1
        # 通道增强
        ch_out = torch.sum(ch_score * ch_Wv, dim=1)
        ...
        # 堆叠空间Wq
        sp_Wq = torch.stack(sp_Wq_outs, dim=-1)  # bs, c//2, d, h, w, k
        # 计算空间Wk各分支
        sp_Wk_outs = [
            conv(ch_out)
            for conv in self.sp_Wk_convs
        ...
        # 堆叠空间Wk
        sp_Wk = torch.stack(sp_Wk_outs, dim=-1)  # bs, c//2, d, h, w, k
        # 计算通道Wv各分支
        sp_Wv_outs = [
            conv(x)
        ...
        # 堆叠通道Wq
        sp_Wv = torch.stack(sp_Wv_outs, dim=-1)  # bs, c, d, h, w, k
        # 转换维度
        sp_Wq = sp_Wq.reshape(bs, c//self.k, -1)  # bs, c//2, d*h*w*k
        sp_Wk = sp_Wk.mean(-1).mean(-1).mean(-1).mean(-1).reshape(bs, 1, -1)  # bs, 1, c//2
        # 进行Softmax处理
        sp_Wk = self.sp_softmax(sp_Wk)  # bs, 1, c//2
        # 矩阵相乘
        sp_Wz = torch.matmul(sp_Wk, sp_Wq)  # bs, 1, d*h*w*k
        # 计算空间注意力分数矩阵
        sp_score = self.sigmoid(sp_Wz.reshape(bs, 1, d, h, w, -1))  # bs, 1, d, h, w, k
        # 空间增强
        sp_out = torch.sum(sp_score * sp_Wv, dim=-1)
